...
class BotUserRepositoryTest @Autowired constructor(
    ...
    /** User(FK) 저장 후 그 유저의 BotUser(botKey, botUserKey, score) 를 저장한다. */
    ...
    @Test
    @DisplayName("같은 botKey 안에서 score 내림차순으로 정렬된다")
    fun ordersByScoreDesc() {
        seed("bot-A", "u1", 100)
        seed("bot-A", "u2", 300)
        seed("bot-A", "u3", 200)

        val ranking = botUserRepository.findTop10ByBotKeyOrderByScoreDesc("bot-A")

        assertEquals(listOf("u2", "u3", "u1"), ranking.map { it.botUserKey })
        assertEquals(listOf(300, 200, 100), ranking.map { it.score })
    }
    ...
    fun limitsToTen() {
        ...
        assertEquals(150, ranking.first().score) // 최고점
        assertEquals(60, ranking.last().score)   // 10등 = 60점(150,140,...,60)
    }
    ...
    fun emptyWhenNoData() {
        seed("bot-A", "a1", 100)
        assertEquals(emptyList(), botUserRepository.findTop10ByBotKeyOrderByScoreDesc("bot-empty"))
    }
}
