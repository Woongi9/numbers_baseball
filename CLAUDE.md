...
- **멘션 프리필 버튼은 `action = "mention"`을 쓴다**(`SkillResponse.Button.mentionPrefill`). 이 액션이 입력창을 `"@봇 "`으로 채워주므로 `messageText`가 필요 없다. 예전엔 `action = "message"` + `messageText`로 흉내냈고, `messageText`가 비었거나 공백이면 카카오가 "값 없음"으로 보고 버튼 `label`로 대체해(예: `"@봇 제출"`) zero-width space를 넣어 우회하던 시절이 있었다 — 그 시절 흔적(상수·주석)이 보이면 낡은 것이다. 한편 `SkillController.play`는 여전히 분류 전에 zero-width 문자를 제거한다(`zeroWidthChars` 정규식). 발화에 그 문자가 섞여 오면 `SkillCommand`의 `all isDigit()` 체크에 조용히 안 걸리므로, 입력 위생 차원의 방어로 남겨둔다.
