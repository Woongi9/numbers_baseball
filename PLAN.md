...
- [ ] **① 카드 이미지 세로 공간 정리** — 진행중 카드(strike/ball/out) 썸네일이 1:1(800×800, `fixedRatio=true`)이라 채팅에서 세로를 많이 차지하고, 연속 판정 시 같은 이미지가 반복돼 보임. **두 방향 중 택1(미결)**:
...
- [ ] **② 자주 쓰는 명령어를 멘션 시 앞 순서로** — 봇 멘션 시 노출되는 명령어(발화 목록) 순서를 사용 빈도 순으로 재배치.
  - 현재: `시작 → 사용법 → 게임 규칙 → 랭킹`
  - 과거: `시작 → 제출 → 랭킹 → 규칙 → 사용법`
