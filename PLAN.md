...
- [x] **정답 축하 연출 강화** — 승리 응답에 이모티콘·강조 문구를 더해 임팩트 부여 (2026-07-05 완료: `SkillController.winHeadline`에서 시도수별(1 / 2~3 / 그 외) 차등 연출)
...
- [X] **(추가 개선) 정답 승리 시 승자 멘션** — 오픈채팅(botKey 있음)에서 승리 응답에 `{{#mentions.winner}} 님, 정답을 맞혔어요!`를 넣고 `extra.mentions.winner = {type:"botUserKey", id:<userId>}`를 실어 승자를 @멘션한다(`botUserKey == userId`). 카드/폴백 양쪽 경로에 동일하게 멘션이 실리도록 `cardOrText`에 `mentions` 파라미터 추가. 1:1/일반챗(botKey 없음)에선 멘션 생략. 테스트: `SkillControllerIntegrationTest.winMentionsWinnerInOpenChat`.
- [X] **(추가 개선) 랭킹 상위 30/20/10% 이모지 강조** — 채팅방 전체 인원(`countByBotKey`)을 모집단으로 각 TOP 10 엔트리의 상위 백분위를 계산해(`PercentileCalculator`) `RankTitle`(🥉 30% / 🥈 20% / 🥇 10%) 뱃지를 이름 앞에 붙인다. TOP 10 안에선 자기보다 높은 사람이 모두 TOP 10에 있어 추가 쿼리 없이 `higher`를 리스트 내에서 산출. 표본 `<MIN_SAMPLE(5)`면 뱃지 없음(기존 백분위 철학 유지). 변경: `BotUserRepository.countByBotKey` 추가, `RankEntry.title` 추가, `RankingService`/`formatRanking` 반영. 테스트: `RankingServiceTest.assignsRankTitleBadge`/`noBadgeBelowMinSample`.
...
## 6-E. 🎨 [2026-07-20 추가] 출시 후 UX 피드백 (카드·안내문 다듬기)

> **배경**: 실제 오픈채팅 플레이 후 받은 UX 피드백. STEP 12(BasicCard)·안내문(도움말/규칙) 위에 얹는 표현 레이어 정리 — 게임 규칙 로직 변경 없음.

- [x] **① 카드 이미지 세로 공간 정리** — 진행중 카드(strike/ball/out) 썸네일이 1:1(800×800, `fixedRatio=true`)이라 채팅에서 세로를 많이 차지하고, 연속 판정 시 같은 이미지가 반복돼 보임. **두 방향 중 택1(미결)**:
  - (A) **이미지 동일 시 제외** — 직전 응답과 같은 썸네일이면 카드에서 이미지를 빼고 TextCard로 (반복 이미지 억제).
  - (B) **이미지 세로폭 축소** — 1:1 대신 2:1 비율로 낮춰 세로 공간 절약.
  - → 결정 필요. 결정 후 `SkillController` 진행 카드 조립부 + `Thumbnail.fixedRatio`/이미지 비율에 반영.
- [x] **② 자주 쓰는 명령어를 멘션 시 앞 순서로** — 봇 멘션 시 노출되는 명령어(발화 목록) 순서를 사용 빈도 순으로 재배치.
  - 현재: `시작 → 사용법 → 게임 규칙 → 랭킹`
  - 과거: `시작 → 제출 → 랭킹 → 규칙 → 사용법`
  - → 자주 쓰는 `시작`/`제출`/`랭킹`을 앞에, `규칙`/`도움말`을 뒤로.
- [ ] **③ 규칙 + 사용법 통합 검토(미결)** — `게임 규칙`(RULES)과 `도움말`(HELP)이 별도 명령·응답. 하나로 합칠 수 있는지 검토 — 합치면 명령/안내문 수가 줄어 ④·⑤와 시너지. → 통합 여부 결정 필요.
- [ ] **④ `사용법` 용어 → `도움말`로 통일** — 사용자 노출 문구/버튼 라벨을 `도움말`로 통일. (참고: `SkillCommand.HELP_WORDS = {"사용법","도움말"}` — 발화 인식은 둘 다 유지, **표기만** 도움말로.)
- [ ] **⑤ 도움말에서 자명한 명령어 설명 제거** — 도움말 본문에서 쉽게 예측되는 명령(예: `시작` = 게임 시작) 설명을 덜어 간결화.

> **한 줄 요약**: 표현 레이어만 손봄. ①(이미지 A/제외 vs B/세로축소)과 ③(규칙+도움말 통합)은 **결정 선행**, ②④⑤는 바로 실행 가능. 게임 로직 무변경.
